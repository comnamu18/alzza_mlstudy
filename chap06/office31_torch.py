...
def train_and_test(model, odata, train_loader, validation_loader, args):
    ...
    for e in range(args.epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            train_metric0 += torch.where(torch.argmax(pred0, axis=1) == y0, 1, 0).sum().item()
            train_metric1 += torch.where(torch.argmax(pred1, axis=1) == y1, 1, 0).sum().item()
            total += y0.size(0)
        ...
        train_metric0 /= total
        train_metric1 /= total
        train_metric = "{:.3f}".format(train_metric0) + '+' + "{:.3f}".format(train_metric1)
        ...
        for i, data in enumerate(validation_loader):
            ...
            val_metric0 += torch.where(torch.argmax(pred0, axis=1) == y0, 1, 0).sum().item()
            val_metric1 += torch.where(torch.argmax(pred1, axis=1) == y1, 1, 0).sum().item()
            total += y0.size(0)
...
if __name__ == '__main__':
    ...
    validation_loader = DataLoader(odata, batch_size=args.batchsize, sampler=validation_sampler)
