...
def init_model(input_cnt, output_cnt, hidden_config=None):
    ...
    if hidden_config is None:
        ...
        print(hidden_config)
        for idx, hidden_cnt in enumerate(hidden_config):
            if idx == 0:
                ...
                new_layer = tf.keras.layers.Dense(
                    hidden_cnt, bias_initializer="zeros", kernel_initializer=initializer
                )
            layers.append(new_layer)
        output_layer = tf.keras.layers.Dense(
            output_cnt, bias_initializer="zeros", kernel_initializer=initializer
        )
        layers.append(output_layer)

    abalone_model = tf.keras.Sequential(layers)
...
def abalone_exec(epoch_count=10, mb_size=10, report=1, hidden_config=None):
    data, input_cnt, output_cnt = load_abalone_dataset()
    abalone_model = init_model(input_cnt, output_cnt, hidden_config)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hidden_config", type=str, default=None, help="List for hiddenconfig"
    )
    args = parser.parse_args()
    hidden_config = None
    if args.hidden_config is not None:
        hidden_config = args.hidden_config.split(",")
    print(hidden_config)
    abalone_exec(hidden_config=args.hidden_config)
